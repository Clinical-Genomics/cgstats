...
def samples(query=None):
    ...
    query = query.join(AnalysisSample.analysis).order_by(Analysis.analyzed_at)
    return query
...
def duplicates():
    ...
    for seq_type in ("wes", "wgs"):
        ...
        percentages = [
            {"name": sample.sample_id, "y": (sample.duplicates_percent * 100)}
            for sample in query
        ]
...
def readsvscov(db):
    ...
    for seq_type in ("wes", "wgs"):
        query = db.query(
            AnalysisSample.reads_total, AnalysisSample.completeness_target_20 * 100
        )
        query = samples(query=query).filter(
            AnalysisSample.sequencing_type == seq_type,
