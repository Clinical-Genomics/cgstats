...
def get_barcode_summary(tree, project, sample, barcode):
    ...
    * OneMismatchBarcodeCount
    ...
    barcodes = xpathsum(
        tree,
        "Flowcell/Project[@name='{project}']/Sample[@name='{sample}']/Barcode[@name='{barcode}']/Lane/BarcodeCount".format(
            project=project, sample=sample, barcode=barcode
        ),
    )
    perfect_barcodes = xpathsum(
        tree,
        "Flowcell/Project[@name='{project}']/Sample[@name='{sample}']/Barcode[@name='{barcode}']/Lane/PerfectBarcodeCount".format(
            project=project, sample=sample, barcode=barcode
        ),
    )
    one_mismatch_barcodes = xpathsum(
        tree,
        "Flowcell/Project[@name='{project}']/Sample[@name='{sample}']/Barcode[@name='{barcode}']/Lane/OneMismatchBarcodeCount".format(
            project=project, sample=sample, barcode=barcode
        ),
    )
...
def get_sample_summary(tree, project, sample, barcode):
    ...
    * pf Q Score
    ...
    """
    raw_clusters = xpathsum(
        tree,
        "Flowcell/Project[@name='{project}']/Sample[@name='{sample}']/Barcode[@name='{barcode}']/Lane/Tile/Raw/ClusterCount".format(
            project=project, sample=sample, barcode=barcode
        ),
    )
    pf_clusters = xpathsum(
        tree,
        "Flowcell/Project[@name='{project}']/Sample[@name='{sample}']/Barcode[@name='{barcode}']/Lane/Tile/Pf/ClusterCount".format(
            project=project, sample=sample, barcode=barcode
        ),
    )
    ...
    pf_yield = xpathsum(
        tree,
        "Flowcell/Project[@name='{project}']/Sample[@name='{sample}']/Barcode[@name='{barcode}']/Lane/Tile/Pf/Read/Yield".format(
            project=project, sample=sample, barcode=barcode
        ),
    )
    log.debug("\tpf_yield={}".format(pf_yield))
    pf_read1_yield = xpathsum(
        tree,
        "Flowcell/Project[@name='{project}']/Sample[@name='{sample}']/Barcode[@name='{barcode}']/Lane/Tile/Pf/Read[@number='1']/Yield".format(
            project=project, sample=sample, barcode=barcode
        ),
    )
    pf_read2_yield = xpathsum(
        tree,
        "Flowcell/Project[@name='{project}']/Sample[@name='{sample}']/Barcode[@name='{barcode}']/Lane/Tile/Pf/Read[@number='2']/Yield".format(
            project=project, sample=sample, barcode=barcode
        ),
    )
    raw_yield = xpathsum(
        tree,
        "Flowcell/Project[@name='{project}']/Sample[@name='{sample}']/Barcode[@name='{barcode}']/Lane/Tile/Raw/Read/Yield".format(
            project=project, sample=sample, barcode=barcode
        ),
    )
    ...
    pf_q30 = xpathsum(
        tree,
        "Flowcell/Project[@name='{project}']/Sample[@name='{sample}']/Barcode[@name='{barcode}']/Lane/Tile/Pf/Read/YieldQ30".format(
            project=project, sample=sample, barcode=barcode
        ),
    )
    ...
    pf_read1_q30 = xpathsum(
        tree,
        "Flowcell/Project[@name='{project}']/Sample[@name='{sample}']/Barcode[@name='{barcode}']/Lane/Tile/Pf/Read[@number='1']/YieldQ30".format(
            project=project, sample=sample, barcode=barcode
        ),
    )
    pf_read2_q30 = xpathsum(
        tree,
        "Flowcell/Project[@name='{project}']/Sample[@name='{sample}']/Barcode[@name='{barcode}']/Lane/Tile/Pf/Read[@number='2']/YieldQ30".format(
            project=project, sample=sample, barcode=barcode
        ),
    )
    # pf_q30_bases_pc = pf_q30 / pf_yield

    pf_qscore_sum = xpathsum(
        tree,
        "Flowcell/Project[@name='{project}']/Sample[@name='{sample}']/Barcode[@name='{barcode}']/Lane/Tile/Pf/Read/QualityScoreSum".format(
            project=project, sample=sample, barcode=barcode
        ),
    )
...
def calc_undetermined(demux_dir):
    ...
    for sample_name, size in sizes.items():
        proc_undetermined[sample_name] = (
            float(size["u_size_of"]) / size["size_of"] * 100
        )
...
def get_raw_clusters_lane(total_sample_summary):
    ...
    """
    raw_clusters_lane = dict(
        zip(
            total_sample_summary.keys(),
            [0 for t in range(len(total_sample_summary.keys()))],
        )
    )  # lane: raw_clusters
...
def parse_samples(demux_dir):
    ...
    for lane in lanes:
        ...
        stats_files = glob.glob(
            "{}/l{}t??/Stats/ConversionStats.xml".format(demux_dir, lane)
        )
        index_files = glob.glob(
            "{}/l{}t??/Stats/DemultiplexingStats.xml".format(demux_dir, lane)
        )
    ...
    for sample in samples:
        ...
        for line in samplesheet.lines_per_column("sample_id", sample):
            ...
            for tree in et_stats_files[lane]:
                summaries[lane][sample].append(
                    get_sample_summary(
                        tree, line["project"], line["sample_name"], line.dualindex
                    )
                )
    ...
    for lane, sample_summary in total_sample_summary.items():
        for sample, summary in sample_summary.items():
            rs[lane][summary["samplename"]] = {
                ...
                "lane": lane,
                "raw_clusters_pc": round(
                    summary["raw_clusters"] / raw_clusters_lane[lane] * 100, 2
                ),
                "pf_clusters": summary["pf_clusters"],
                "pf_yield_pc": round(
                    summary["pf_yield"] / summary["raw_yield"] * 100, 2
                ),
                ...
                "undetermined_pc": (summary["pf_clusters"] - summary["barcodes"])
                / summary["pf_clusters"]
                * 100,
