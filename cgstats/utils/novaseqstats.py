...
def get_barcode_summary(tree, project, sample, barcode, lane):
    ...
    * OneMismatchBarcodeCount
    ...
    barcodes = xpathsum(
        tree,
        f"Flowcell/Project[@name='{project}']/Sample/"
    ...
    for sample in samples:
        ...
        for line in samplesheet.lines_per_column("sample_id", sample):
            ...
            summaries[lane][sample].append(
                get_sample_summary(
                    et_stats_file, line["project"], line["sample_name"], barcode, lane
                )
            )

            summaries[lane][sample].append(
                get_barcode_summary(
                    et_index_file, line["project"], line["sample_name"], barcode, lane
                )
            )
    ...
    for lane, sample_summary in total_sample_summary.items():
        for sample, summary in sample_summary.items():
            rs[lane][summary["samplename"]] = {
                ...
                if summary["pf_read1_yield"]
                else 0,
                "pf_read2_q30": round(
                    summary["pf_read2_q30"] / summary["pf_read2_yield"] * 100, 2
                )
                if summary["pf_read2_yield"]
                else 0,
...
def parse(unaligned_dir):
    ...
    for lane in lanes:
        ...
        summaries[lane].append(
            get_sample_summary(et_stats_file, "all", "all", "all", lane)
        )
