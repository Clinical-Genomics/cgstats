...
def flowcells(sample=None):
    ...
    query = (
        Flowcell.query.join(Demux).join(Datasource).order_by(Datasource.rundate.desc())
    )
    if sample:
        pattern = SAMPLE_PATTERN.format(sample)
        query = query.join(Demux.unaligned, Unaligned.sample).filter(
            or_(Sample.samplename.like(pattern), Sample.samplename == sample)
        )
    return query
...
def select(flowcell, project_name=None):
    ...
    query = query.with_entities(
        ...
        func.group_concat(Unaligned.readcounts.op("ORDER BY")(Unaligned.lane)).label(
            "reads"
        ),
        func.sum(Unaligned.readcounts).label("readsum"),
        func.group_concat(Unaligned.yield_mb.op("ORDER BY")(Unaligned.lane)).label(
            "yld"
        ),
        func.sum(Unaligned.yield_mb).label("yieldsum"),
        func.group_concat(
            func.truncate(Unaligned.q30_bases_pct, 2).op("ORDER BY")(Unaligned.lane)
        ).label("q30"),
        func.group_concat(
            func.truncate(Unaligned.mean_quality_score, 2).op("ORDER BY")(
                Unaligned.lane
            )
        ).label("meanq"),
    )
